...
def prepare_pickle(dataroot, picklefile, participant):
    assert participant in ("main-agent", "interloctr"), "`participant` must be either 'main-agent' or 'interloctr'"
    ...
    for i, filename in enumerate(filenames):
        ...
        g_data["has_finger"] = hasfinger
        g_data["speaker_id"]= speaker_id
        ...
        with open(os.path.join(picklefile, filename + '.pickle'), 'wb') as f:
...
def prepare_h5_unclipped(dataroot, h5file, participant):
    assert participant in ("main-agent", "interloctr"), "`participant` must be either 'main-agent' or 'interloctr'"    
    ...
    with h5py.File(h5file, "w") as h5:
        for i, filename in enumerate(filenames):
            ...
            sentence = np.array(sentence, dtype='S')
            g_data.create_dataset("text", data=sentence)
            print(filename)
    print()
    return
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--dataset_path", type=str, default="F://GENEA2023//genea2023_dataset")
    ...
    prepare_pickle(dataroot, f"{save_path}//{dataset_type}_main-agent_v0//", "main-agent")
    prepare_pickle(dataroot, f"{save_path}//{dataset_type}_interloctr_v0//", "interloctr")