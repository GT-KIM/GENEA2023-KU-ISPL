...
if __name__ == '__main__':
    ...
    """
    bvhfiles = glob.glob('F://GENEA2023//genea2023_dataset//val//main-agent//bvh//*.bvh')
    save_dir = "..//outputs//ref//npy//"
    ...
    for bvhfile in sorted(bvhfiles):
        ...
        out_file = bvhfile.split("\\")[-1]
        #out_file = bvhfile.replace('.bvh', '.npy').replace('/bvh/', '/npy/')
        np.save(os.path.join(save_dir, out_file), npy_data)
    """
    #bvhfiles = glob.glob('..//outputs_train//bvh//prepose//*.bvh')
    #save_dir = "..//outputs_train//npy//prepose//"
    bvhfiles = glob.glob('..//outputs_entry//NA//*.bvh')
    save_dir = "..//outputs_entry//npy//NA//"
    os.makedirs(save_dir, exist_ok=True)
    for bvhfile in sorted(bvhfiles):
        print(bvhfile)
        npy_data = convert_bvh(bvhfile)
