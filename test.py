...
from dataloader.diffusion_test import Diffusiondataset, tst_collate_fn
...
def main(config, pre_config):
    ...
    with open("outputs_train/mean_pose.txt", 'r') as f:
        t = ''.join(f.readlines())
        mean_pose = ast.literal_eval(t)

    dataset = Diffusiondataset(config, pre_config, phase='test')
    dataloader = DataLoader(dataset, batch_size=1, shuffle=False, collate_fn = tst_collate_fn)

    logger.log("creating model and diffusion...")
    model, diffusion = create_model_and_diffusion(use_prepose=True,
    ...
    for file_idx, batch in enumerate(dataloader) :
        ...
        for idx, sample in enumerate(samples) :
            ...
            if idx == 0 :
                pre_pose = th.tensor([mean_pose for _ in range(8)]).to('cuda:0')
            pre_pose = pre_pose.unsqueeze(0)

            audio, text = th.tensor(audio).float().to('cuda:0'), th.tensor(text).float().to('cuda:0')
            ...
            sample = sample_fn(
                ...
                progress=True,
                model_kwargs={'pre_pose' : pre_pose, 'audio' : audio, 'text' : text},
            )
...
def convert(predicted_gesture, name, config, structure="full") :
    ...
    writer = BVHWriter()
    os.makedirs(os.path.join(config.outputpath, "prepose"), exist_ok=True)
    with open(os.path.join(config.outputpath, "prepose", name + '.bvh'), 'w') as f:
...
def create_argparser():
    defaults = dict(
        ...
        batch_size=1,
        use_ddim=False,
        model_path="C://Users//PC//PycharmProjects_2023//GENEA2023//ckpts//diffusion//model361000.pt",
    )
